...
LLM 에이전트를 만들려는 분을 위해 한 줄 주석을 달아 둔 읽기 목록으로, 서베이 <i>“LLM Agents: A Survey”</i>와 짝을 이룹니다.
</p>
...
| | 담긴 내용 |
|---|---|
| 📚 **서베이 그 이상** | *LLM Agents: A Survey*가 인용한 참고문헌 228편에, 논문이 확정된 뒤로 검증해 추가한 연구까지 더했습니다. 논문은 한 시점의 기록이지만 이 목록은 계속 자랍니다. |
...
**LLM 기반 에이전트**의 필독 논문을 모은 저장소입니다. 여기서 LLM 기반 에이전트란 계획, 메모리, 도구 사용, 다중 에이전트 협조 능력을 갖추고 긴 호흡으로 목표를 추구하는 언어 모델을 말합니다. 논문은 함께 나온 서베이의 분류 체계를 따라 에이전트의 핵심 구성 요소, 에이전트가 쓰이는 환경과 응용, 그리고 평가와 안전처럼 분야 전반에 걸친 과제 순으로 정리했습니다. 항목마다 논문 링크를 달았고, 공식 구현이 있으면 코드 링크도 함께 걸었습니다.

이 목록은 **엄선해 계속 갱신하는 목록**입니다. 서베이의 참고문헌을 그대로 옮긴 것이 아니라, 서베이를 빠짐없이 포함하면서 그보다 넓습니다. 논문이 인용한 참고문헌 228편은 하나하나 원출처로 확인했고, 이 목록은 논문이 확정된 뒤에 나온 연구를 같은 원칙으로 확인해 더하면서 그 수를 훌쩍 넘어섰습니다. 섹션은 기본으로 접혀 있으며, **논문 N편 보기**를 누르면 펼쳐집니다.
...
| 8 | [LLM-based Multi-Agents: A Survey](https://arxiv.org/abs/2402.01680) | 다중 에이전트 | 다중 에이전트 갈래를 볼 때 표준으로 삼는 참고문헌입니다. |
| 9 | [AgentBench: Evaluating LLMs as Agents](https://arxiv.org/abs/2308.03688) | 평가 | 여러 환경을 가로질러 에이전트를 재는 표준 벤치마크로 자리 잡았습니다. |
...
- **[Toolformer: Language Models Can Teach Themselves to Use Tools](https://arxiv.org/abs/2302.04761)** (Schick et al., NeurIPS 2023) - *LLM 에이전트 분류 체계에서 '도구 확장'이라는 축을 떠받치는, 도구 사용 분야의 선구적 논문이다.* ⭐
...
- **[Agent AI: Surveying the Horizons of Multimodal Interaction](https://arxiv.org/abs/2401.03568)** (Durante et al., arXiv 2024) - *LLM 에이전트 서베이의 지형을 multimodal·embodied 에이전트 AI까지 넓혔다.*
...
- **[Understanding the Planning of LLM Agents: A Survey](https://arxiv.org/abs/2402.02716)** (Huang et al., arXiv 2024) - *기초 서베이들이 비워 둔 계획 분야를 채운다.*
...
- **[Anatomy of Agentic Memory: Taxonomy and Empirical Analysis of Evaluation and System Limitations](https://arxiv.org/abs/2602.19320)** (Jiang et al., arXiv 2026) - *에이전트 메모리 구조를 분류하고, 여러 시스템에 걸친 벤치마크 포화와 지표 타당성 문제를 실험으로 드러낸다.*
...
- **[Agents That Know Too Much: A Data-Centric Survey of Privacy in LLM Agents](https://arxiv.org/abs/2606.26627)** (Lahjouji et al., arXiv 2026) - *에이전트 프라이버시 연구를 공격 유형이 아니라 데이터가 드러나는 지점별로 정리한 데이터 중심 서베이로, 거버넌스의 빈틈을 짚는다.*
...
- **[From Question Answering to Task Completion: A Survey on Agent System and Harness Design](https://arxiv.org/abs/2606.20683)** (Guo et al., arXiv 2026) - *에이전트를 모델 대 harness라는 관점에서 읽는다. Harness를 실행 시점의 책임 여섯 가지로 나누고, 성능 병목이 실제로 어디에 있는지 묻는다.* [[code](https://github.com/ggjy/Awesome-Agent-Engineering)]
...
- **[Always-On Agents: A Survey of Persistent Memory, State, and Governance in LLM Agents](https://arxiv.org/abs/2606.30306)** (Ding et al., arXiv 2026) - *세션을 넘어 지속되는 상태를 가진 에이전트 연구 435편을 정리하고, 문헌이 축적과 검색에는 치우치면서 거버넌스와 복구는 소홀히 한다는 점을 밝힌다.*
- **[Externalization in LLM Agents: A Unified Review of Memory, Skills, Protocols and Harness Engineering](https://arxiv.org/abs/2604.08224)** (Zhou et al., arXiv 2026) - *에이전트의 발전을 외부화로 보고, 능력이 가중치 밖으로 빠져나와 메모리, skill, 프로토콜, harness 인프라로 옮겨 가는 흐름을 그린다.*
- **[SoK: Agentic Skills -- Beyond Tool Use in LLM Agents](https://arxiv.org/abs/2602.20867)** (Jiang et al., arXiv 2026) - *재사용할 수 있는 호출형 절차로서의 agentic skill을 체계화하고, skill과 원자적 도구 호출 사이에 선을 긋는다.*
...
- **[Uncertainty Quantification in LLM Agents: Foundations, Emerging Challenges, and Opportunities](https://arxiv.org/abs/2602.05073)** (Oh et al., arXiv 2026) - *불확실성 정량화가 단일 턴 QA를 벗어나 상호작용하는 에이전트로 옮겨 가야 한다고 주장하며, 일반적인 정식화와 함께 에이전트에만 있는 네 가지 과제를 제시한다: 추정기 선택, 이질적인 개체, 불확실성의 동역학, 세밀한 벤치마크의 부재.*
- **[Multi-Agent Debate Strategies: Survey, Taxonomy, and Challenges](https://arxiv.org/abs/2607.26212)** (Motger et al., arXiv 2026) - *다중 에이전트 토론 연구 141편을 검토해, 이 분야가 통제된 비교가 아니라 관행에 따라 투표를 곁들인 정적 fully connected 토폴로지에 조용히 자리 잡았음을 밝힌다.*
...
- **[Blockchain Empowered Trustworthy Agent Networks: Foundations, Taxonomy, and Future Directions](https://arxiv.org/abs/2608.04626)** (Zhu et al., arXiv 2026) - *1980년부터 2026년까지 고전적 다중 에이전트 시스템에서 개방형 에이전트 네트워크에 이르는 흐름을 정리한다. 서로 다른 주체가 소유한 에이전트끼리 거래하기 시작하면 신뢰 문제가 네트워크 수준으로 옮겨 가고, 단일 에이전트용 안전 장치로는 거기에 닿을 수 없다고 주장한다.*
...
- **[Agent Safety Should Be a Runtime Contract](https://arxiv.org/abs/2608.11274)** (Ng et al., arXiv 2026) - *에이전트 안전을 harness 안의 런타임 계약으로 두자는 포지션 페이퍼로, 이 계약에는 예방하는 면(sandbox, 권한 게이트, 모니터)과 증거를 남기는 면(테스트 실행, 로그 수집, 파일 diff)이 있다. 기록된 사고 52건, 12개 공개 에이전트 시스템의 trajectory 스키마 감사, NeurIPS·ICML·ICLR 2023-2025 논문 28,560편 전체의 제목 수준 감사를 근거로 삼으며, 마지막 감사에서는 학습 시점 연구와 배포 시점 연구 사이에 합산 기준 8-12배의 출판 불균형이 드러났다.*
- **[Information Retrieval Misses the Mark for LLM Agents](https://doi.org/10.2139/ssrn.6903579)** (Sun et al., SSRN 2026) - *실제 배포 현장을 "RAG는 끝났고 에이전트에게는 grep이면 된다"로 읽는 시각에 답하는 포지션 페이퍼다. 기반이 바뀌었다는 점은 인정하지만, 어긋남은 다섯 차원에 걸친 구조적 문제라고 본다. IR이 전제하는 코퍼스, 입력, 목표, 에피소드, 검색 대상이 모두 계획하고 브라우징하고 도구를 부르며 검색을 계속할지 스스로 정하는 에이전트에게는 맞지 않기 때문이다. 에이전트를 고정한 채 HotpotQA-distractor와 2WikiMultihopQA에서 BM25, 벡터, grep, 하이브리드, closed-book 검색을 바꿔 가며 이 격차를 시험하고, 검색을 상태에 따라 증거를 모으는 정책으로 다시 정의하자고 주장한다.*
- **[Terminal Agents: A Survey of AI Agents in Command-Line Environments](https://arxiv.org/abs/2608.20485)** (Bin et al., arXiv 2026) - *과제 영역이 아니라 터미널을 정리의 축으로 삼는다. 명령 실행과 텍스트 피드백을 거쳐 진척을 내는 루프를 가진 에이전트를 아키텍처, 역량 획득, 평가에 걸쳐 일곱 차원의 역량 프로파일 위에 배치한다. 조건을 고정한 자체 진단에서는 벤치마크 계열마다 드러나는 과정 신호가 다르고 조건을 맞춘 시스템 비교도 벤치마크에 따라 결과가 갈려, 어떤 결과든 단일 구성 요소의 공으로 돌릴 수 있는 범위가 제한된다.*
...
- **[Gorilla: Large Language Model Connected with Massive APIs](https://arxiv.org/abs/2305.15334)** (Patil et al., NeurIPS 2024) - *Fine-tuning에 검색을 더하면 에이전트가 방대한 실제 API 목록을 안정적으로 호출할 수 있음을 보인, 단일 에이전트 도구 사용의 핵심 논문이다.* [[code](https://github.com/ShishirPatil/gorilla)]
...
- **[Tree of Thoughts: Deliberate Problem Solving with Large Language Models](https://arxiv.org/abs/2305.10601)** (Yao et al., NeurIPS 2023) - *LATS 같은 후속 단일 에이전트 계획·탐색 프레임워크의 토대가 된, 숙고형 탐색 추론의 핵심 아키텍처다.* [[code](https://github.com/princeton-nlp/tree-of-thought-llm)]
...
- **[MetaGPT: Meta Programming for A Multi-Agent Collaborative Framework](https://arxiv.org/abs/2308.00352)** (Hong et al., ICLR 2024) - *단일 에이전트의 역할·절차 템플릿이 어떻게 신뢰성을 높이는지 보여 준, 널리 인용되는 프레임워크다. 단일 에이전트에서 다중 에이전트로 넘어가는 프레임워크 설계의 전환점에 있다.* [[code](https://github.com/geekan/MetaGPT)]
...
- **[Orchestral AI: A Framework for Agent Orchestration](https://arxiv.org/abs/2601.02577)** (Roman et al., arXiv 2026) - *전문화된 에이전트들을 하나의 인터페이스 뒤에서 조합하고 조율하는 프레임워크다.* [[code](https://github.com/orchestralAI/orchestral-ai)]
...
- **[Next-Generation Agentic Reinforcement Learning Systems Enable Self-Evolving Agents](https://arxiv.org/abs/2607.01120)** (Ran Yan et al., arXiv 2026) - *가중치만이 아니라 자기 구성 요소까지 고치는 에이전트를 가능하게 하는 것은 알고리즘이 아니라 agentic RL 시스템 스택이라고 주장한다.*
...
- **[Self-Evolving World Models for LLM Agent Planning](https://arxiv.org/abs/2606.30639)** (Zhang et al., arXiv 2026) - *실행기는 고정한 채 테스트 시점에 에이전트 내부의 world model을 다듬어, 계획하고 시뮬레이션하는 루프를 더 촘촘하게 만든다.*
- **[Scaling Self-Evolving Agents via Parametric Memory](https://arxiv.org/abs/2606.04536)** (Ren et al., arXiv 2026) - *자기 진화를 context window 밖, 파라미터 안으로 옮겨, 프롬프트를 늘리지 않고도 쌓인 경험이 확장되게 한다.*
...
- **[Argus: A General-Purpose Agentic Runtime for Long-Horizon Reasoning](https://arxiv.org/abs/2608.05144)** (Li et al., arXiv 2026) - *Manager, Planner, Engineer, Reviewer가 오래 유지되는 프로젝트 상태 위에서 범위가 정해진 임무를 수행하는 지속형 런타임이다. 메모리, skill, 검증기는 담당 역할의 검토를 거친 뒤에만 받아들인다. 가중치는 그대로이므로, SWE-Bench Pro에서 모델을 바로 쓰는 baseline이 59%일 때 78%를 낸 것은 전적으로 런타임 상태 덕분이다.*
- **[Architectural Implications of Agentic AI Workflows](https://arxiv.org/abs/2608.04458)** (Yang et al., arXiv 2026) - *Azure의 실제 운영 환경을 조사해 agentic 워크로드를 데이터센터 문제로 규정한다. Orchestration과 도구 때문에 CPU가 critical path에 올라서고, 단편화된 실행이 기존의 균일한 서버에서 CPU와 GPU 용량을 모두 놀게 만든다는 점을 밝힌다.*
...
- **[STaR: Bootstrapping Reasoning With Reasoning](https://arxiv.org/abs/2203.14465)** (Zelikman et al., NeurIPS 2022) - *에이전트에 스스로 생성한 추론을 심는 데 쓰이는 RL 기반 reasoning model 학습 패러다임(예: DeepSeek-R1, o1)의 전신이다.* [[code](https://github.com/ezelikman/STaR)]
- **[Graph of Thoughts: Solving Elaborate Problems with Large Language Models](https://arxiv.org/abs/2308.09687)** (Besta et al., AAAI 2024) - *에이전트가 쓰는 구조화된 추론·탐색 프레임워크를 트리 너머로 확장해, 복잡한 다단계 과제에서 품질과 비용을 모두 개선한다.* [[code](https://github.com/spcl/graph-of-thoughts)]
- **[Reasoning with Language Model is Planning with World Model](https://arxiv.org/abs/2305.14992)** (Hao et al., EMNLP 2023) - *탐색으로서의 고전적 계획(MCTS, world model)을 LLM 추론과 연결한다. 불확실성 아래에서 에이전트가 계획하는 문제와 바로 맞닿아 있다.* [[code](https://github.com/maitrix-org/llm-reasoners)]
...
- **[Large Language Models Cannot Self-Correct Reasoning Yet](https://arxiv.org/abs/2310.01798)** (Huang et al., ICLR 2024) - *에이전트 설계에서 self-critique 루프에 지나치게 기대는 경향에 제동을 건, 널리 인용되는 경고성·비판적 결과다. 외부 피드백에 근거한 방법이 나오는 계기가 되었다.*
...
- **[Recursive Self-Improvement in AI: From Bounded Self-Refinement to Autonomous Research Loops](https://arxiv.org/abs/2607.07663)** (Chen et al., arXiv 2026) - *범위가 제한된 self-refinement부터 자율 연구 루프까지의 스펙트럼을 정리하고, 재귀적 자기 개선의 각 단계가 지금 어디서 무너지는지 짚는다.*
...
- **[SearchMaster: Grounded and Regulated Self-Play for Search Agents](https://arxiv.org/abs/2608.01822)** (Tan et al., arXiv 2026) - *검색 에이전트를 위한 self-play로, 스스로 만든 데이터가 잘못 이끄는 세 가지 경로를 짚는다: 가짜 multi-hop 질문을 막는 증거 사슬, 성공률이 아닌 검색 깊이로 매기는 난이도, 에이전트가 끝내 쓰지 않는 문서를 여는 데 주는 페널티.* [[code](https://github.com/WentaoTan/SearchMaster)]
- **[R³-Bench: LLMs Struggle with Resource-Rational Reasoning under Shared Budgets](https://arxiv.org/abs/2608.16033)** (Wang et al., arXiv 2026) - *여섯 문제로 된 묶음이 연산 예산을 함께 쓰는 조건에서, 같은 모델의 조건을 맞춘 단일 문제 응답 곡선으로 만든 오프라인 oracle이 72개 칸 모두에서 모델의 실제 묶음 점수와 같거나 높았고 71개 칸에서는 엄밀히 더 높았다. 단일 문제에서 보여 준 역량과 공유 예산 아래에서 실제로 내는 성과 사이에 틈이 있다는 뜻이다. Trajectory 진단에서는 전략을 거의 고쳐 나가지 못하는 모습이 보였다.* [[code](https://github.com/NineAbyss/R-3-Bench)]
...
- **[Do GUI Agents Know When Not to Act? Enabling Conflict-Aware Termination for Multimodal GUI Agents](https://arxiv.org/abs/2609.03438)** (Huang et al., arXiv 2026) - *행동하는 결정이 아니라 멈추는 결정을 벤치마크로 잰다. 스스로 모순되는 지시와 화면 내용과 모순되는 지시를 모두 다루며, 실행 쪽으로 치우친 과잉 순응을 발견한다: 실행 가능한 과제에서 점수가 좋은 에이전트도 지시가 충돌하는 과제에서 계속 실행한다. 추론 시점의 실행 가능성 점검에 행동 조절을 더하면 에이전트 다섯 개 모두에서 이 문제가 줄고, 일반 과제 성능도 떨어지지 않는다.*
- **[Steer, Don't Solve: Training Small Critic Models for Large Code Agents](https://arxiv.org/abs/2606.21811)** (Gandhi et al., arXiv 2026) - *4B·8B critic 모델을 SFT와 DPO로 학습시켜, 코딩 에이전트의 trajectory에서 오류를 찾고 추론 시점에 몇 단계마다 상위 수준의 방향을 제시하게 한다. 행동을 직접 생성하지는 않는다. 이 critic은 더 큰 에이전트 여섯 개의 SWE-bench Verified 해결률을 높였는데, GLM-4.7-Flash-30B-A3B는 16.0점, GPT-OSS-120B는 14.4점 올랐다. 에이전트가 더 적은 단계로 끝나는 경우에는 조언에 드는 비용을 스스로 메워, GPT-OSS-20B의 예시당 비용이 $0.07에서 $0.03으로 줄었다.* [[code](https://github.com/shubhamrgandhi/critic-training)]
...
- **[A-MEM: Agentic Memory for LLM Agents](https://arxiv.org/abs/2502.12110)** (Xu et al., arXiv 2025) - *노트를 그래프로 서로 이어 가며 스스로 동적으로 조직되는, 최신 세대 LLM 에이전트 long-term memory 시스템의 대표 사례이다.* [[code](https://github.com/WujiangXu/A-mem)]
...
- **[MIRIX: Multi-Agent Memory System for LLM-Based Agents](https://arxiv.org/abs/2507.07957)** (Wang et al., arXiv 2025) - *LLM 기반 에이전트를 위한 다중 에이전트·다중 유형(multimodal) 메모리 아키텍처라는, 지금 가장 앞선 흐름을 보여 준다.* [[code](https://github.com/Mirix-AI/MIRIX)]
...
- **[ReasoningBank: Scaling Agent Self-Evolving with Reasoning Memory](https://arxiv.org/abs/2509.25140)** (Ouyang et al., arXiv 2025) - *성공한 trajectory와 실패한 trajectory 양쪽에서 전략 수준의 메모리를 뽑아내, 과제 흐름을 따라 에이전트가 자기 진화하게 한다.*
...
- **[MemOS: A Memory OS for AI System](https://arxiv.org/abs/2507.03724)** (Li et al., arXiv 2025) - *메모리를 일급 자원(MemCube)으로 끌어올리고, 파라미터 메모리, 활성값 메모리, 평문 메모리를 스케줄링과 거버넌스를 맡는 하나의 운영체제 아래에 묶는다.* [[code](https://github.com/MemTensor/MemOS)]
...
- **[Agentic Context Engineering: Evolving Contexts for Self-Improving Language Models](https://arxiv.org/abs/2510.04618)** (Zhang et al., ICLR 2026) - *생성, reflection, 정리로 이어지는 delta 연산으로 컨텍스트 자체를 playbook처럼 키워 가며, 자기 개선 에이전트가 겪는 간결성 편향과 컨텍스트 붕괴를 막는다.* [[code](https://github.com/ace-agent/ace)]
...
- **[MINTEval: Evaluating Memory under Multi-Target Interference in Long-Horizon Agent Systems](https://arxiv.org/abs/2605.18565)** (Lee et al., arXiv 2026) - *QA 15.6K개로 이루어진 벤치마크(컨텍스트 최대 1.8M 토큰)로, 서로 간섭하며 자주 갱신되는 사실 앞에서 메모리 에이전트가 고전한다는 것을 보여 준다.* [[code](https://github.com/amy-hyunji/MINTEval)]
...
- **[MemSyco-Bench: Benchmarking Sycophancy in Agent Memory](https://arxiv.org/abs/2607.01071)** (Xiang et al., arXiv 2026) - *에이전트 메모리의 아첨 성향, 곧 저장된 믿음이 세션을 거치며 사용자의 압력에 굽히는지를 재는 벤치마크이다.* [[code](https://github.com/XMUDeepLIT/MemSyco-Bench)]
- **[Remember the Decision, Not the Description: A Rate-Distortion Framework for Agent Memory](https://arxiv.org/abs/2605.10870)** (Zou et al., arXiv 2026) - *에이전트 메모리를 rate-distortion 관점으로 정식화해, 명시적인 왜곡 예산 아래에서 설명이 아니라 결정을 남긴다.*
...
- **[COMFYCLAW: Self-Evolving Skill Harnesses for Image Generation Workflows](https://arxiv.org/abs/2607.01709)** (Li et al., arXiv 2026) - *이미지 생성 workflow 구축을 타입이 있는 그래프 편집으로 정식화하고, vision-language model로 시각적 오류를 찾아 고치며, 지난 실행을 distillation으로 정리해 재사용할 skill 라이브러리를 계속 키워 가는 agentic 프레임워크를 제안한다. 에이전트 구성 여섯 가지 모두에서 평균 점수가 가장 높고, skill 진화 없이 검증기만 쓰는 baseline도 앞선다.*
...
- **[Filesystem-Based Memory for LLM Agents: Organization, Evolution, and Sustainability](https://arxiv.org/abs/2607.26637)** (Zhou et al., arXiv 2026) - *실제로 배포되는 에이전트가 쓰는 markdown 디렉터리 메모리는 검색을 경제적으로 만들지만(큰 자료에서 검색 비용이 대략 절반) 더 나은 답을 주지는 못하고, 저장소가 커질수록 구조가 허물어진다는 것을 밝혔다.*
...
- **[When Memory Lies: An Empirical Study of Spatial Memory Staleness in VLM Agents](https://arxiv.org/abs/2608.04574)** (Sun et al., arXiv 2026) - *확신을 담아 저장된 메모리가 에이전트가 눈으로 보는 것과 어긋날 때 무슨 일이 생기는지 잰다. 같은 격자에서도 시각 F1은 0.887에서 0.067까지 벌어지고, 메모리를 그대로 믿는 에이전트는 메모리를 아예 받지 않은 같은 에이전트보다 두 배 넘게 자주 죽는다.*
- **[Caching for the Future: Scrub Jay Episodic Memory Principles for Agent Memory Systems](https://arxiv.org/abs/2608.04746)** (Bhandari et al., arXiv 2026) - *덤불어치의 episodic memory에서 유형에 따른 망각을 빌려 와, 저장 항목마다 부패도 계수와 효용 유지 기간을 붙여 낡은 사실이 검색에서 서서히 빠지게 한다. 감쇠 항을 빼면 일반화 성능이 5.7배 무너진다.*
...
- **[ForeDreamer: A Self-Evolving Dual-Agent Memory Architecture for Future Event Prediction](https://arxiv.org/abs/2608.20920)** (Zhong et al., EMNLP Findings 2026) - *검색 결과를 에이전트에 바로 넣지 않고, 예측에 앞서 날것의 웹 근거를 구조화된 메모리로 바꾼다. 질문별 사실 메모리와 예측 에피소드를 넘어 이어지는 경험 메모리를 나누어 앞의 것은 메모리 하위 에이전트가 만들고, 두 갈래의 진화 과정이 예측과 메모리 구축을 함께 개선한다. Prophet Arena와 FutureX에서 평가했다.* [[code](https://github.com/zhongzero/ForeDreamer)]
...
- **[Does Your Agent's Memory Survive a Model Upgrade? A Controlled Study of Memory Portability](https://arxiv.org/abs/2609.05339)** (Goyal et al., arXiv 2026) - *저장된 이력은 그대로 두고 모델만 바꿔, 업그레이드를 견딜 수 있는지는 메모리 형식이 정한다는 것을 보인다. 고정 스키마 지식 그래프는 정확도가 0.0004포인트 움직이는 데 그치지만, 모델이 압축한 노트는 마이그레이션 방향에 따라 +9.91 또는 -13.28포인트로 비대칭하게 흔들린다. 절반만 마이그레이션한 임베딩 색인은 전체 재임베딩이 되찾는 11.90포인트 가운데 4.96포인트만 회복하고, 원본을 남겨 두지 않았다면 노트를 저장소 안에서만 고치는 방식은 48개 이력 모두에서 90% 회복 목표에 못 미친다.*
- **[The Memory Trust Gap: Capability-Dependent Failures in Persistent-Memory Agents](https://arxiv.org/abs/2609.01852)** (Hu et al., arXiv 2026) - *메모리가 유일한 출처인 경우와 권위 있는 도구가 올바른 값을 쥐고 있는 경우를 나누고, 실패를 혼동이 아니라 과신으로 읽는다. 메모리밖에 없을 때 모델은 0.92에서 1.00의 비율로 낡은 저장값에 기대 답한다. 함정 조건의 피해는 능력에 따라 갈려서, Qwen3 크기 계열 가운데 큰 모델일수록 낡은 노트가 최신인 척 꾸며지면 가장 크게 무너진다.*
...
- **[TALM: Tool Augmented Language Models](https://arxiv.org/abs/2205.12255)** (Parisi et al., arXiv 2022) - *언어 모델이 도구 사용을 자기 지도 방식으로 스스로 익혀 가게 하는 초기 정식화로 영향력이 컸고, Toolformer의 자기 지도 접근을 직접 앞서 보여 주었다.*
...
- **[RestGPT: Connecting Large Language Models with Real-World RESTful APIs](https://arxiv.org/abs/2306.06624)** (Song et al., arXiv 2023) - *장난감 수준의 도구 모음을 넘어, 상태를 가진 복잡한 실제 REST API로 도구 사용을 넓혀 보였다. 함께 낸 벤치마크는 지금도 API 기반 에이전트 평가에 쓰인다.* [[code](https://github.com/Yifan-Song793/RestGPT)]
...
- **[StableToolBench: Towards Stable Large-Scale Benchmarking on Tool Learning of Large Language Models](https://arxiv.org/abs/2403.07714)** (Guo et al., ACL 2024) - *널리 쓰이는 평가 인프라 논문으로, 실제 API를 쓰는 대규모 도구 학습 벤치마크를 괴롭히던 재현성 문제를 해결했다.* [[code](https://github.com/THUNLP-MT/StableToolBench)]
...
- **[HyperTool: Beyond Step-Wise Tool Calls for Tool-Augmented Agents](https://arxiv.org/abs/2606.13663)** (Du et al., arXiv 2026) - *결정적인 도구 서브루틴을 모델이 코드로 한 번 내리는 바깥 호출 하나로 접어 넣어, 중간값이 추론 trace를 거치지 않고 로컬에서 오가게 한다. MCP-Universe에서 Qwen3-32B의 성능을 15.7%에서 35.3%로 끌어올렸다.*
- **[Tool-Aware Optimization with Entropy Guidance for Efficient Agentic Reinforcement Learning](https://arxiv.org/abs/2606.03762)** (Cao et al., arXiv 2026) - *도구 호출이 모두 실패했거나 결과가 한결같이 맞거나 틀린 rollout을 버리고, 도구 호출 직후 토큰에 엔트로피 보너스를 더해 중요한 결정 지점에서 탐색이 살아 있게 함으로써 agentic RL을 안정시킨다.*
...
- **[CodeDelegator: Mitigating Context Pollution via Role Separation in Code-as-Action Agents](https://arxiv.org/abs/2601.14914)** (Fei et al., arXiv 2026) - *전략적 계획을 맡아 계속 살아 있는 Delegator 에이전트와, 깨끗한 컨텍스트에서 하위 과제를 실행하는 새 Coder 에이전트를 분리한 CodeDelegator를 제안한다.*
...
- **[ToolAtlas: Learning Once, Reusing Everywhere with Tool-Side Memory](https://arxiv.org/abs/2607.11126)** (Fang et al., arXiv 2026) - *메모리를 에이전트가 아니라 도구 제공자 쪽에 둔다. 실제로 실행해 확인한 도구의 능력, 실패 경계, 조합 기록이 pass@1을 최대 21.61% 올리고, 다시 학습하지 않고도 다른 에이전트 프레임워크로 옮겨 간다.*
- **[The Bitter Lesson of Tool Calling](https://arxiv.org/abs/2608.06370)** (Patel et al., arXiv 2026) - *BFCL v4에서 모델 14개를 대상으로 코드로 도구를 부르는 방식과 기본 JSON 방식을 비교한다. 도구를 타입이 붙은 Python stub으로 노출하고 모델이 코드로 호출하게 하면 14개 중 11개에서 JSON과 같거나 낫고, GPT-5.6 계열에서는 10.6% 오르며, baseline이 흔들리는 context rot 아래에서도 버틴다.*
- **[Diagnosing Tool-Selection Reasoning in LLM Agents with Canary Tools](https://arxiv.org/abs/2608.04719)** (Anand et al., arXiv 2026) - *MCP 도구 모음에 진단용 미끼 도구를 심어, 틀린 도구를 골랐다는 결과만 남던 자리에 그 이유의 프로필이 남게 한다. 여섯 가지 탐침 유형과 8,640회 실행에서 모델 간 취약도는 약 36배 차이 나며, 능력 등급과 비례하지 않는다.*
...
- **[Thinking With Tools, Not With Pixels: Tool Calls as Text Scaffolds for Visual Reasoning](https://arxiv.org/abs/2608.09682)** (Shao et al., arXiv 2026) - *자르기·확대 도구가 돌려주는 이미지를 텍스트 placeholder로 바꿔도 LoRA, full fine-tuning, RL 모두에서 이미지를 온전히 쓰는 thinking-with-images와 같거나 더 낫다. 성능을 떠받치는 신호는 돌아온 픽셀이 아니라 호출할 때 내놓는 구조화된 텍스트라는 뜻이다. 지연 시간은 29에서 46%까지 줄고 도구 실행 API 호출은 사라진다.*
- **[Can MCP Clients Decide What to Do After Failure? A Result-Only Actionability Audit](https://arxiv.org/abs/2609.00072)** (Mehan, arXiv 2026) - *끝난 MCP 실패 결과 하나만 보고 결정론적 소프트웨어가 무엇을 판단할 수 있는지 묻는다. 일부러 작게 잡은 표본(접근 가능한 서버 열 곳에서 유도한 실패 21건)에서 타입이 있는 필드는 18건에서 실패를 드러내고 8건에서 대략적인 대응 방침을 읽게 해 주지만, 구체적인 원인, 대상, 실행 가능한 수정, 재실행 제약은 한 번도 드러내지 않는다. 그래서 복구는 누군가 해석해야 하는 산문에 기대게 된다.*
...
- **[Encouraging Divergent Thinking in Large Language Models through Multi-Agent Debate](https://arxiv.org/abs/2305.19118)** (Liang et al., EMNLP 2024) - *다중 에이전트 토론이 self-consistency를 넘어 도움이 되는 이유를 뒷받침하는 핵심 실패 유형을 진단한다.* [[code](https://github.com/Skytliang/Multi-Agents-Debate)]
...
- **[Scaling Large Language Model-based Multi-Agent Collaboration](https://arxiv.org/abs/2406.07155)** (Qian et al., arXiv 2024) - *명시적 topology 위에서 협업을 1000개 넘는 에이전트로 확장하고, 불규칙한 그래프가 규칙적인 그래프보다 낫다는 협업 scaling 결과를 보고한다.*
...
- **[When is Routing Meaningful? Diversity and Robustness in Language Model Societies](https://arxiv.org/abs/2607.09197)** (Huot et al., arXiv 2026) - *여러 모델로 이루어진 사회에서 라우팅이 언제 의미가 있는지, 다양성이 언제 잡음이 아니라 강건성을 가져오는지 묻는다.*
- **[What LLM Agents Say When No One Is Watching: Social Structure and Latent Objective Emergence in Multi-Agent Debates](https://arxiv.org/abs/2607.02507)** (Ghaffarizadeh et al., arXiv 2026) - *지켜보는 이가 없을 때 에이전트들이 무슨 말을 하는지 관찰해, 과제 지표가 놓치는 잠재 목표와 사회 구조를 드러낸다.*
...
- **[The Long-Horizon Task Mirage? Diagnosing Where and Why Agentic Systems Break](https://arxiv.org/abs/2604.11978)** (Wang et al., arXiv 2026) - *Long-horizon 과제에서 agentic 시스템이 어디서 왜 무너지는지 진단하고, 겉보기의 long-horizon 능력 상당 부분이 신기루라고 주장한다.*
...
- **[A Case Study on Emergent Cheating and Whistleblowing in Autonomous Research Swarms](https://arxiv.org/abs/2609.04170)** (Paglieri et al., arXiv 2026) - *형식 추측을 증명하는 에이전트 100개 집단에서 에이전트 하나가 평가 시스템의 허점을 찾아냈고, 이 허점은 경쟁 압력 속에서 공유 지식 라이브러리를 거쳐 에이전트 간 메시지로 퍼졌다. 한편 다른 무리는 부정한 증명을 점검하고, 동료에게 경고하고, 보이콧을 벌이고, 검증 패치를 제안했다. 허점을 실어 나른 투명한 채널이 저항을 가능하게 한 바로 그 채널이었고, 저자들은 이를 개별 에이전트 내부의 실패가 아니라 공유지 거버넌스 문제로 읽는다.*
</details>
...
- **[CogAgent: A Visual Language Model for GUI Agents](https://arxiv.org/abs/2312.08914)** (Hong et al., arXiv 2023) - *스크린샷만으로 GUI grounding을 하도록 만든 초기 대형 VLM 가운데 하나로, 고해상도 시각 GUI 에이전트 아키텍처 계열을 열었다.* [[code](https://github.com/zai-org/CogAgent)]
...
- **[PhoneBuddy: Training Open Models for Agentic Phone Use](https://arxiv.org/abs/2606.23049)** (Tang et al., arXiv 2026) - *비공개 시스템이 장악해 온 agentic 휴대폰 조작 분야를 위해 공개 모델을 학습시킨다.* [[code](https://github.com/PhoneBuddyAI/phonebuddy)]
- **[Designing Agent-Ready Websites for AI Web Agents: A Framework for Machine Readability, Actionability, and Decision Reliability](https://arxiv.org/abs/2607.12056)** (Elnaffar et al., arXiv 2026) - *문제를 거꾸로 뒤집어, 웹사이트를 어떻게 만들어야 에이전트가 기계적으로 읽어 내고 조작할 수 있는지 묻는다.*
...
- **[WebNavigator: Global Web Navigation via Interaction Graph Retrieval](https://arxiv.org/abs/2603.20366)** (Zhang et al., arXiv 2026) - *상호작용 그래프를 검색하며 웹을 탐색해, 에이전트에게 페이지 단위의 국소적 시야 대신 전역 구조를 준다.* [[code](https://github.com/fate-ubw/webNavigator)]
...
- **[ASPIRE: Agentic /Skills Discovery for Robotics](https://arxiv.org/abs/2607.00272)** (Lu et al., arXiv 2026) - *Code-as-policy 패러다임 아래 에이전트가 로봇 제어 프로그램을 스스로 쓰고 다듬는 지속 학습 시스템 ASPIRE를 제시한다.*
...
- **[StepReflect: Structured UI Transition Reflection for Mobile GUI Agents](https://arxiv.org/abs/2608.05587)** (Guo et al., arXiv 2026) - *단계별 GUI reflection을 열린 multimodal 추론이 아니라 명시적인 전이 명세 위의 구조화된 예측으로 다룬다. 8B 모델이 AndroidWorld에서 전이 정확도 82.16%를 기록해, 같은 입력을 받은 zero-shot GPT-5.2보다 11.83포인트 높다.*
- **[ComponentBench: Diagnosing Component-Level Failures in Computer-Use Agents](https://arxiv.org/abs/2608.18307)** (Guan et al., arXiv 2026) - *대표 UI 컴포넌트 97종의 온톨로지 위에 프로그램으로 검증한 과제 2,910개를 두고 computer use 에이전트를 컴포넌트 수준에서 평가한다. 공유하는 harness 하나 안에서 관측과 action space만 바꿔도 같은 모델의 과제 성공률이 30% 넘게 달라지며, GPT-5 mini는 accessibility tree 관측에서 83.1%이던 것이 좌표만 쓰는 픽셀 제어에서는 48.9%로 떨어진다.* [[code](https://github.com/TianchenGuan/ComponentBench)]
...
- **[CUA-Universe: A Scalable and Dynamic Environment for Hybrid GUI+CLI Agents](https://arxiv.org/abs/2609.05374)** (Shi et al., arXiv 2026) - *같은 애플리케이션 상태에 화면으로도 명령줄로도 닿을 수 있는 환경을 만든다. 실제 데스크톱 애플리케이션 16개를 재현 가능한 가상 머신으로 옮기고, 명령 인터페이스는 찾아내거나 감싸거나 새로 생성해 붙인다. 여기서 모은 trajectory로 학습하면 9B 모델이 비효율적인 클릭과 깨지기 쉬운 스크립트에서 벗어나 더 싼 쪽 인터페이스를 고르게 되어, OSWorld에서 성공률이 16.8포인트 오르고 단계는 57%, 토큰은 44% 줄어든다.*
- **[Discriminative World Models for Web Agents](https://arxiv.org/abs/2609.02885)** (Li et al., arXiv 2026) - *다음 상태 예측으로 지도학습한 world model은 순위 모델이 그 출력을 쓰는 순간 엉뚱한 목표에 최적화된 셈이라고 주장한다. 순위를 매기려면 그럴듯해 보이는 상태가 아니라 후보 행동들을 서로 가려 주는 예측 상태가 필요하기 때문이다. 그래서 결정 지점마다 대안 행동과 각 행동이 만드는 상태가 함께 달린, 가지를 치는 WebArena trajectory 위에서 예측 상태 매칭으로 학습한다.*
...
- **[Towards Scientific Intelligence: A Survey of LLM-based Scientific Agents](https://arxiv.org/abs/2503.24047)** (Ren et al., arXiv 2025) - *과학 발견 에이전트 문헌의 토대를 잡아 주는 세부 주제 전용 서베이다.*
...
- **[FinGPT: Open-Source Financial Large Language Models](https://arxiv.org/abs/2306.06031)** (Yang et al., IJCAI 2023) - *가장 많이 인용되는 오픈소스 금융 LLM·에이전트 프로젝트 중 하나로, 여러 후속 금융 에이전트 시스템이 딛고 선 기반 모델 인프라를 제공한다.* [[code](https://github.com/AI4Finance-Foundation/FinGPT)]
...
- **[OptimAI: Optimization from Natural Language Using LLM-Powered AI Agents](https://arxiv.org/abs/2504.16918)** (Thind et al., arXiv 2025) - *자연어로 적힌 최적화 문제를 formulator, planner, coder, critic으로 이어지는 파이프라인에 태워 실행 가능한 solver 코드로 바꾸고, 계획은 UCB 기반으로 고른다.*
...
- **[LawThinker: A Deep Research Legal Agent in Dynamic Environments](https://arxiv.org/abs/2602.12056)** (Yang et al., arXiv 2026) - *변화하는 법률 환경에서 움직이는 deep research 법률 에이전트다.* [[code](https://github.com/RUC-NLPIR/LawThinker-agent)]
...
- **[HealthAgentBench: A Unified Benchmark Suite of Realistic Agentic Healthcare Environments for Challenging Frontier AI Agents](https://arxiv.org/abs/2606.31179)** (Liu et al., arXiv 2026) - *정적인 임상 질의응답이 아니라 에이전트가 실제로 움직이는 현실적인 의료 환경으로 구성한 벤치마크 모음이다.* [[code](https://github.com/microsoft/HealthAgentBench)]
...
- **[SWE-bench: Can Language Models Resolve Real-World GitHub Issues?](https://arxiv.org/abs/2310.06770)** (Jimenez et al., ICLR 2024) - *코딩·소프트웨어 공학 에이전트의 사실상 표준 벤치마크이며, SWE-bench Verified/Lite/Live/Multimodal 계열을 낳았다.* [[code](https://github.com/SWE-bench/SWE-bench)]
- **[MLAgentBench: Evaluating Language Agents on Machine Learning Experimentation](https://arxiv.org/abs/2310.03302)** (Huang et al., arXiv 2023) - *'AI 연구 에이전트'·'ML 엔지니어링 에이전트' 평가라는 하위 분야를 연 벤치마크로, MLE-bench, RE-Bench 등의 전신이다.* [[code](https://github.com/snap-stanford/MLAgentBench)]
- **[τ-bench: A Benchmark for Tool-Agent-User Interaction in Real-World Domains](https://arxiv.org/abs/2406.12045)** (Yao et al., arXiv 2024) - *에이전트와 사용자의 상호작용, 그리고 정책 준수를 평가하는 길을 처음 열었고, 기업·고객 서비스 에이전트 평가에서 표준 참조점이 되었다.* [[code](https://github.com/sierra-research/tau-bench)]
...
- **[TravelPlanner: A Benchmark for Real-World Planning with Language Agents](https://arxiv.org/abs/2402.01622)** (Xie et al., ICML 2024) - *제약이 많은 복잡한 다중 도구 계획을 시험하는 스트레스 테스트로 널리 인용되며, 에이전트가 믿을 만한 long-horizon 계획에서 아직 얼마나 멀리 있는지 보여 준다.* [[code](https://github.com/OSU-NLP-Group/TravelPlanner)]
...
- **[τ²-Bench: Evaluating Conversational Agents in a Dual-Control Environment](https://arxiv.org/abs/2506.07982)** (Barres et al., arXiv 2025) - *사용자와 에이전트가 모두 환경에 행동을 가하는 설정으로 τ-bench를 확장한다.* [[code](https://github.com/sierra-research/tau2-bench)]
...
- **[Beyond Static Leaderboards: Predictive Validity for the Evaluation of LLM Agents](https://arxiv.org/abs/2606.19704)** (Patel et al., arXiv 2026) - *에이전트 순위표가 배포에 관해 의미 있는 말을 하려면 점수만이 아니라 예측 타당도가 필요하다고 주장한다.*
...
- **[Measuring Harness-Induced Belief Divergence in Multi-Step LLM Agents](https://arxiv.org/abs/2607.04528)** (Yi et al., arXiv 2026) - *Harness 하나만으로 에이전트의 믿음이 단계를 거치며 얼마나 달라지는지 측정해, 에이전트 평가에 숨은 교란 요인을 분리해 낸다.* [[code](https://github.com/Hik289/Harness-induce-bias)]
- **[Rethinking the Evaluation of Harness Evolution for Agents](https://arxiv.org/abs/2607.12227)** (Wang et al., arXiv 2026) - *Harness도 모델만큼 점수를 움직인다는 점에서, harness의 진화를 어떻게 평가해야 할지 다시 따져 본다.* [[code](https://github.com/rethinking-harness-evolution/code)]
...
- **[AgentAtlas: Beyond Outcome Leaderboards for LLM Agents](https://arxiv.org/abs/2605.20530)** (Mazaheri et al., arXiv 2026) - *에이전트 평가를 결과 순위표 너머로 옮겨, 제어가 어디서 무너지는지를 결정 단위로 진단한다.*
...
- **[UniClawBench: A Universal Benchmark for Proactive Agents on Real-World Tasks](https://arxiv.org/abs/2607.08768)** (Chen et al., arXiv 2026) - *실제로 돌아가는 Docker 환경에서 단계별 체크포인트를 두고 실행하는 과제 400개로 이루어진, 능력 중심의 이중 언어 벤치마크 UniClawBench를 내놓는다. 능동형 LLM 에이전트를 skill 활용, 탐색, 긴 컨텍스트 추론, multimodal 이해, 여러 플랫폼 간 조율에 걸쳐 평가하며, 숨겨진 감독 에이전트를 두어 다중 턴 피드백으로 채점 기준이 새어 나가지 않게 한다.* [[code](https://github.com/HKU-MMLab/UniClawBench)]
...
- **[OmnilingualGAIA2: Evaluating the Multilingual Gap in Frontier AI Agents](https://arxiv.org/abs/2608.08775)** (Caciolai et al., arXiv 2026) - *GAIA2를 다섯 가지 문자 체계에 걸친 열 개 언어로 넓히자 pass@3 기준 8.8에서 18.4점의 언어 간 격차가 드러났다. 이 격차는 정량 추론이 아니라 도구 orchestration에 몰려 있고 모델 규모를 키워도 좁혀지지 않는다. Error attribution으로는 격차의 55%가 모델에서 비롯되며, 번역 contamination은 시나리오·언어 쌍의 6.4% 이내로 한정된다.*
...
- **[AgentDojo: A Dynamic Environment to Evaluate Prompt Injection Attacks and Defenses for LLM Agents](https://arxiv.org/abs/2406.13352)** (Debenedetti et al., NeurIPS 2024) - *Prompt injection 공격과 방어 앞에서 에이전트가 얼마나 견고한지 재는, 가장 널리 쓰이는 표준 테스트베드다.* [[code](https://github.com/ethz-spylab/agentdojo)]
...
- **[BadAgent: Inserting and Activating Backdoor Attacks in LLM Agents](https://arxiv.org/abs/2406.03007)** (Wang et al., ACL 2024) - *에이전트 backdoor가 이후의 안전 fine-tuning을 거쳐도 살아남는다는 것을 선구적으로 보여 주며, 에이전트 공급망 보안 문제에 불을 붙였다.* [[code](https://github.com/DPamK/BadAgent)]
...
- **[SafeAgentBench: A Benchmark for Safe Task Planning of Embodied LLM Agents](https://arxiv.org/abs/2412.13178)** (Yin et al., arXiv 2024) - *에이전트 안전 평가를 디지털·텍스트 영역 너머 물리 세계의 embodied 위험으로 넓혀, 안전 실패가 로보틱스로도 이어진다는 것을 보였다.* [[code](https://github.com/shengyin1224/SafeAgentBench)]
- **[AI Agents That Matter](https://arxiv.org/abs/2407.01502)** (Kapoor et al., arXiv 2024) - *분야가 에이전트 능력 주장을 평가하는 방식을 바꿔 놓은, 널리 인용되는 비판이다. 에이전트의 위험과 이득 사이의 균형을 믿을 만하게 평가하는 문제와 직결된다.*
...
- **[AI Alignment: A Comprehensive Survey](https://arxiv.org/abs/2310.19852)** (Ji et al., arXiv 2023) - *가장 포괄적인 일반 정렬 서베이 중 하나로, 에이전트 특화 안전 연구가 딛고 서는 개념적 scaffold(RICE, forward/backward alignment)를 제공한다.* [[code](https://github.com/PKU-Alignment/AlignmentSurvey)]
...
- **[Discovering Language Model Behaviors with Model-Written Evaluations](https://arxiv.org/abs/2212.09251)** (Perez et al., arXiv 2022) - *RLHF 학습 규모를, 자기 보존이나 권력 추구에 가까운 선호를 드러내는 경향이 새로 나타나는 현상과 연결한 초기의 중요한 실증 근거로, 자율 에이전트 정렬 문제를 미리 예고했다.* [[code](https://github.com/anthropics/evals)]
...
- **[Provably Secure Agent Guardrail](https://arxiv.org/abs/2605.29251)** (Wu et al., arXiv 2026) - *에이전트가 행동하기 전에 의도를 일차 논리 제약으로 형식화하도록 강제해, 공격 성공과 오탐을 하나도 남기지 않는다.*
...
- **[When Agents Remember Too Much: Memory Poisoning Attacks on Large Language Model Agents](https://arxiv.org/abs/2607.06595)** (Torres et al., arXiv 2026) - *에이전트를 노리는 memory poisoning 공격을 다룬다. 오래 남는 상태를 오염시켜 침해가 세션이 끝난 뒤에도 살아남게 한다.*
...
- **[Prismata: Confining Cross-Site Prompt Injection in Web Agents](https://arxiv.org/abs/2607.08147)** (Villa et al., arXiv 2026) - *웹 에이전트의 cross-site prompt injection을, 모델이 버텨 주리라 믿는 대신 경계에서 가둔다.*
...
- **[Overcoming the Retrieval Barrier: Indirect Prompt Injection in the Wild for LLM Systems](https://arxiv.org/abs/2601.07072)** (Chang et al., arXiv 2026) - *현장에서 벌어지는 indirect prompt injection을 연구해, 실제 LLM 시스템에서는 검색 장벽이 가정보다 약하다는 것을 보인다.*
...
- **[KidnapRAG: A Black-Box Attack for Hijacking Reasoning in Agentic Retrieval-Augmented Generation Systems](https://arxiv.org/abs/2607.00422)** (Choi et al., arXiv 2026) - *프롬프트, trace, 파라미터에 전혀 접근하지 않고 역할이 정해진 문서 세 개(Bait, Chain-Link, Mal-Ins)로 agentic RAG를 black-box poisoning한다. 세 문서는 각각 첫 검색을 끌어들이고, 질의 재작성을 유도하고, 공격자가 통제하는 근거를 공급한다.* [[code](https://github.com/chanwoochoi316/KidnapRAG)]
- **[(A)I Sees What You Don't: Exploiting New Attack Surfaces in Third-Party Mobile Agents](https://arxiv.org/abs/2607.00333)** (Zhang et al., arXiv 2026) - *VLM으로 움직이는 서드파티 모바일 에이전트에서 지금까지 정리되지 않은 attack surface 두 가지를 찾아낸다. 하나는 사람과 기계가 화면을 다르게 보는 데서 생기는 화면 인식 surface이고, 다른 하나는 에이전트의 실행 파이프라인을 가로채거나 조작하는 오용 채널 surface이다. 인기 프레임워크 다섯 개에서 구체적인 공격 일곱 가지를 보였는데, 특별한 권한이 없는 악성 앱도 에이전트의 행동을 가로챌 수 있고 심하면 임의 명령 실행까지 이어진다.*
...
- **[Protocol-Level Attacks on Agentic Commerce Platforms: A Cross-Platform Taxonomy, AIP-Bench, and Unified Defense](https://arxiv.org/abs/2607.21824)** (Louck, arXiv 2026) - *에이전트 커머스 보안을 한 계층 아래로 내린다. 플랫폼 세 곳에 걸친 프로토콜 취약점 33개는 모델과 상관없이 결정론적으로 악용되며, 그중 세 개를 엮으면 end-to-end 결제 탈취가 된다.*
...
- **[Rethinking MCP Security: A Large-Scale Study of Runtime MCP Servers and Security Scanner Reliability](https://arxiv.org/abs/2607.11086)** (Chen et al., arXiv 2026) - *실사용 중인 MCP 서버 64,611개를 모았고 그중 37,000개 이상이 실행 가능했다. 이를 감사하는 스캐너들은 양치기 소년처럼 헛경보를 울려, 표본 경보 가운데 수작업 검증을 통과한 것이 절반도 되지 않았다.*
...
- **[LoginTrap: Uncovering Task-Agnostic Phishing-Style Indirect Prompt Injection Attacks against LLM-based Web Agents](https://arxiv.org/abs/2608.04741)** (Guo et al., arXiv 2026) - *웹 에이전트를 말로 구슬려 로그인하게 만들 수 있음을 보인다. 공격자가 통제하는 페이지 내용이 인증을 과제의 전제 조건처럼 보이게 해 에이전트를 공격자의 로그인 페이지로 이끌며, 사용자의 과제를 모르고도 평균 86%의 end-to-end 성공률을 낸다.*
...
- [js-lee-AI/awesome-agent-loop-papers](https://github.com/js-lee-AI/awesome-agent-loop-papers): **이 목록의 자매 목록**으로, 한 단계 더 내려가 에이전트 루프 자체를 다룹니다. 제어 전략, 학습된 루프, skill, harness, 그리고 루프가 만들어 내는 평가와 안전 문제를 모았습니다. ![stars](https://img.shields.io/github/stars/js-lee-AI/awesome-agent-loop-papers?style=social)
- [Hannibal046/Awesome-LLM](https://github.com/Hannibal046/Awesome-LLM): 대규모 언어 모델 논문, 도구, 자료가 모이는 대표 허브입니다. ![stars](https://img.shields.io/github/stars/Hannibal046/Awesome-LLM?style=social)
...
- [luo-junyu/Awesome-Agent-Papers](https://github.com/luo-junyu/Awesome-Agent-Papers): 서베이와 짝을 이루는 목록으로, 분류 체계에 따라 에이전트의 구축, 협업, 진화를 다룹니다. ![stars](https://img.shields.io/github/stars/luo-junyu/Awesome-Agent-Papers?style=social)
- [e2b-dev/awesome-ai-agents](https://github.com/e2b-dev/awesome-ai-agents): 오픈소스와 클로즈드 소스 **에이전트 제품과 프레임워크**(만드는 쪽)를 이미지와 영상까지 곁들여 폭넓게 모은 디렉터리입니다. ![stars](https://img.shields.io/github/stars/e2b-dev/awesome-ai-agents?style=social)
...
한 단계 더 깊이 들어간 자매 서베이 *The Agent Loop: A Survey of Control Strategies, Skills, and Harnesses for LLM Agents*는 별도의 기록이며, DOI는 [`10.2139/ssrn.7186738`](https://ssrn.com/abstract=7186738)입니다. 실제로 참고하신 쪽을 인용해 주세요.
...
오류 수정, 더 날카로운 설명, 아예 새로운 절도 똑같이 환영합니다. 항목 형식은 **[CONTRIBUTING.md](CONTRIBUTING.md)** 파일에 정리해 두었습니다.
...
- **2026-09-16**: 기여자 표도 각 절처럼 토글 안에 넣었고, 표의 인원수도 `sync_counts.py`가 정확하게 맞춥니다.
- **2026-09-07**: 최신 논문 반영: 2026년 5월 이후 올라온 논문 전체에서 절마다 두 편씩, 모두 +20편을 추가했습니다. LoopArena는 저자 중 한 명인 [@burgerseater](https://github.com/burgerseater)가 [#14](https://github.com/js-lee-AI/awesome-llm-agent-papers/issues/14)에서 제안한 논문입니다. 이번 묶음은 부정적 결과 쪽으로 기울어 있는데, 쓸모 있는 연구가 나온 곳도 바로 그쪽이었습니다. 비교 기준을 rollout 수가 아니라 추론 비용으로 맞추는 순간 본전을 뽑지 못하는 다중 에이전트 구조, 고정 스키마는 거의 움직이지 않는데 모델을 바꾸면 열세 점이나 흔들리는, 모델이 직접 쓴 메모리 노트, 기능 테스트는 통과했지만 리뷰어라면 돌려보낼 패치 644개 중 221개, 프롬프트로 하지 말라고 해도 실행의 57%에서 심어 둔 지름길을 택하는 frontier 에이전트가 그 예입니다. 497편에서 517편으로 늘었습니다.
- **2026-09-03**: 세 편 모두에 저자로 참여한 [@dukesun99](https://github.com/dukesun99)가 보내 준 논문 세 편입니다([#13](https://github.com/js-lee-AI/awesome-llm-agent-papers/pull/13)). Corpus2Skill은 메모리에, OrchMAS는 다중 에이전트 시스템에, Information Retrieval Misses the Mark for LLM Agents는 서베이와 포지션 페이퍼에 넣었습니다. 마지막 논문은 이 목록에 처음 실린 SSRN 게재물로, CONTRIBUTING은 arXiv나 DOI 링크를 요구할 뿐 호스트를 제한하지 않으니 규칙에 맞습니다. 세 설명 모두 병합 전에 초록을 바탕으로 다시 썼고, Corpus2Skill 설명은 이제 데이터셋 열한 개에 걸친 결과, 곧 코퍼스 탐색이 그중 세 개에서 뒤진다는 결과까지 보여 줍니다. 494편에서 497편으로 늘었습니다.
- **2026-08-25**: 저자 중 한 명인 [@zhongzero](https://github.com/zhongzero)가 ForeDreamer를 메모리에 추가했습니다([#12](https://github.com/js-lee-AI/awesome-llm-agent-papers/pull/12)). 이제 설명은 이 시스템이 무엇을 다르게 하는지부터 말합니다. 검색 결과를 곧바로 에이전트에 넣는 대신, 예측하기 전에 웹 근거를 구조화된 메모리로 바꿔 둔다는 점입니다. 493편에서 494편으로 늘었습니다.
- **2026-08-20**: 최신 논문 반영: 2026년 8월 논문에서 절마다 두 편씩, 모두 +20편을 추가했습니다. 코드 링크는 논문이 자기 저장소를 밝힌 경우에만 달았습니다. 이달 논문은 부정적 결과 쪽으로 기울었는데, 이 목록과 잘 맞는 성향입니다. MCP와 CLI를 비교해 scaffold가 결과를 좌우한다는 것을 밝힌 연구, 과제 완료율은 제자리인데 검색은 세 배로 늘어나는 압축, 알고 보니 불필요했던 도구 반환 이미지, 규모를 키워도 좁혀지지 않는 다국어 격차가 그렇습니다. 473편에서 493편으로 늘었습니다.
- **2026-08-20**: 병합한 커뮤니티 기여는 Dr. Bench([@reacher-z](https://github.com/reacher-z), [#7](https://github.com/js-lee-AI/awesome-llm-agent-papers/pull/7))와 CityReal([@Nicolas99-9](https://github.com/Nicolas99-9), [#10](https://github.com/js-lee-AI/awesome-llm-agent-papers/pull/10)) 두 건입니다. CityReal PR은 절 제목과 토글 요약의 숫자는 고쳤지만 목차 줄은 그대로 두어서, 브랜치에서 숫자를 먼저 맞추고 병합했습니다. 471편에서 473편으로 늘었습니다.
...
- **2026-08-08**: 최신 논문 반영: 2026년 8월 논문 +16편을 열 개 절 전체에 걸쳐 추가했고, 공식 저장소가 있으면 링크를 달았습니다. 451편에서 467편으로 늘었습니다.
- **2026-08-08**: 앞선 소급 보강 작업에서 논문 설명 아홉 개가 문장 도중에 잘렸고, 그중 세 개는 괄호가 열린 채 끝나 있었습니다. 아홉 개 모두 논문의 초록을 바탕으로 다시 썼고, Agon에는 [@WhymustIhaveaname](https://github.com/WhymustIhaveaname)이 [#5](https://github.com/js-lee-AI/awesome-llm-agent-papers/pull/5)에서 알려 준 `[code]` 링크를 달았습니다. 이런 문제는 이제 `scripts/check_glosses.py`가 검사합니다.
...
- **2026-07-31**: 최신 논문 반영: 2026년 7월 논문을 절마다 세 편씩, 모두 +30편 추가했고 공식 저장소가 있으면 링크를 달았습니다. 421편에서 451편으로 늘었습니다.
- **2026-07-26**: ClawBench 관리에 참여하고 있는 [@reacher-z](https://github.com/reacher-z)가 ClawBench를 평가와 벤치마크에 추가했습니다. 420편에서 421편으로 늘었습니다.
- **2026-07-25**: 첫 커뮤니티 기여입니다. [@jinmang2](https://github.com/jinmang2)가 에이전트 메모리 시스템 +6개(MemoryOS, Zep, Nemori, MemOS, G-Memory, ACE)를, [@WhymustIhaveaname](https://github.com/WhymustIhaveaname)이 연구 에이전트·orchestration 논문 +3편(AutoNumerics, OptimAI, PerspectiveGap)을 보내 주었습니다. 병합 전에 제목, 저자, 학회, arXiv ID, 코드 링크를 모두 다시 확인했습니다.
- **2026-07-19**: 2026년 종합 소급 보강: 검증을 마친 논문 +78편(2026년 1월부터 7월까지)을 열 개 절 전체에 더 추가했고, 확인된 경우에만 공식 저장소 링크를 달았습니다.
- **2026-07-19**: 2026년 1월부터 5월까지의 소급 보강: +30편(절마다 세 편)을 추가했고, 공식 저장소가 있으면 링크를 달았습니다.
- **2026-07-16**: 최신 논문 반영: 2026년 6월과 7월 논문 +50편을 열 개 절 전체에 추가했고, 공식 저장소가 있으면 링크를 달았습니다.
- **2026-07-12**: 2026년 논문 일괄 반영: 열 개 절 전체에 +42편을 추가하고, 실시간 스타 수를 보여 주는 **주목할 10편 (2026)** 절을 새로 만들었으며, 관련 목록에는 실시간 스타 배지를 달았습니다. 211편에서 253편으로 늘었습니다.
- **2026-07-09**: 문헌 갱신: +27편(agentic RL, 프로토콜, deep research, frontier 평가와 안전)을 추가했습니다. 184편에서 211편으로 늘었습니다.
